#include "hub75.h"
#include "pins.h"
#include "delay.h"
#include <math.h>
#include <string.h>

int hubBrightness = 50; //Brightness 2..100%; 0..1% - off
int hubMinBrightness = 2;

orient_t hubOrientation = HUB_ROTATE_0; //Display Orientation
char hubNeedRedraw = false;
	
screen_t matrix[SCREEN_PAGES];
screen_t * screen = &matrix[0], * display = &matrix[1], * screen_temp;

screen_t matrix[SCREEN_PAGES] = { 0 };

const pin_t
	hub_r1 = {
		GPIOC,
		GPIO_PIN_11
},
	hub_g1 = {
		GPIOC,
		GPIO_PIN_12
},
	hub_b1 = {
		GPIOC,
		GPIO_PIN_13
},
	hub_r2 = {
		GPIOD,
		GPIO_PIN_2
},
	hub_g2 = {
		GPIOD,
		GPIO_PIN_4
},
	hub_b2 = {
		GPIOD,
		GPIO_PIN_5
},
	hub_a = {
		GPIOE,
		GPIO_PIN_2
},
	hub_b = {
		GPIOE,
		GPIO_PIN_3
},
	hub_c = {
		GPIOE,
		GPIO_PIN_4
},
	hub_d = {
		GPIOE,
		GPIO_PIN_5
},
	hub_clk = {
		GPIOD,
		GPIO_PIN_7
},
	hub_lat = {
		GPIOG,
		GPIO_PIN_2
},
	hub_noe = {
		GPIOF,
		GPIO_PIN_6
},
	hub_time = {
	GPIOA,
	GPIO_PIN_5
};
	
#define HUB_GPIO_CLK_ENABLE() \
	__GPIOC_CLK_ENABLE(); \
	__GPIOD_CLK_ENABLE(); \
	__GPIOE_CLK_ENABLE(); \
	__GPIOG_CLK_ENABLE(); \
	__GPIOF_CLK_ENABLE()

#define NOE_TIM_CHANNEL TIM_CHANNEL_1

TIM_HandleTypeDef hubtim, hubtim2;

static void initIO(void) {
	HUB_GPIO_CLK_ENABLE();
	
	GPIO_InitTypeDef gpioInit;

	gpioInit.Mode = GPIO_MODE_OUTPUT_PP;
	gpioInit.Pull = GPIO_PULLUP;
	gpioInit.Speed = GPIO_SPEED_HIGH;

	pinSet(hub_noe);
	gpioInit.Pin = hub_noe.pin;
	HAL_GPIO_Init(hub_noe.port, &gpioInit);
	pinSet(hub_noe);
	
	gpioInit.Pin = hub_r1.pin;
	HAL_GPIO_Init(hub_r1.port, &gpioInit);
	gpioInit.Pin = hub_b1.pin;
	HAL_GPIO_Init(hub_b1.port, &gpioInit);
	gpioInit.Pin = hub_g1.pin;
	HAL_GPIO_Init(hub_g1.port, &gpioInit);
	gpioInit.Pin = hub_r2.pin;
	HAL_GPIO_Init(hub_r2.port, &gpioInit);
	gpioInit.Pin = hub_g2.pin;
	HAL_GPIO_Init(hub_g2.port, &gpioInit);
	gpioInit.Pin = hub_b2.pin;
	HAL_GPIO_Init(hub_b2.port, &gpioInit);
	gpioInit.Pin = hub_a.pin;
	HAL_GPIO_Init(hub_a.port, &gpioInit);
	gpioInit.Pin = hub_b.pin;
	HAL_GPIO_Init(hub_b.port, &gpioInit);
	gpioInit.Pin = hub_c.pin;
	HAL_GPIO_Init(hub_c.port, &gpioInit);
	gpioInit.Pin = hub_d.pin;
	HAL_GPIO_Init(hub_d.port, &gpioInit);
	gpioInit.Pin = hub_clk.pin;
	HAL_GPIO_Init(hub_clk.port, &gpioInit);
	gpioInit.Pin = hub_lat.pin;
	HAL_GPIO_Init(hub_lat.port, &gpioInit);
	gpioInit.Pin = hub_time.pin;
	HAL_GPIO_Init(hub_time.port, &gpioInit);
	
	/**TIM10 GPIO Configuration    
   PF6     ------> TIM10_CH1 => NOE
  */
  gpioInit.Pin = GPIO_PIN_6;
  gpioInit.Mode = GPIO_MODE_AF_PP;
  gpioInit.Pull = GPIO_PULLUP;
  gpioInit.Speed = GPIO_SPEED_HIGH;
  gpioInit.Alternate = GPIO_AF3_TIM10;
  HAL_GPIO_Init(GPIOF, &gpioInit);
}

static int period0, period1, periodDark;

#define HUB_PRESCALER (HUB_TIMER_MHZ >> 1) // 500nS - for tick
#define FRAME_PERIOD (HUB_TIMER_MHZ * 1000000 / HUB_PRESCALER / HUB_UPDATE_HZ)
#define PERIOD0 (HUB_TIMER_MHZ * HUB_PERIOD0_US / HUB_PRESCALER)

static int prescaler2N[HUB_COLOR_DEPTH];
static int prescaler2n;
static int period1N[HUB_COLOR_DEPTH];
static int period1n;
static int period2Min, period2Max;

#define P1_KOEFF(x) (100 * (x) / 100 ) // equal to p2 

static inline void setBrightness(void) {
	int sum = 0, p1, p2, bit, k;
	if (hubBrightness < hubMinBrightness) hubBrightness = 0;
	else if (hubBrightness > 100) hubBrightness = 100;
	//periodDark = GET_PERIOD_DARK();
	period2Max = HUB_PERIOD2_MAX_NS / HUB_TIMER2_NS * hubBrightness / 100;
	period2Min = HUB_PERIOD2_MIN_NS / HUB_TIMER2_NS;

	if (period1 < HUB_MIN_PERIOD) period1 = HUB_MIN_PERIOD;
	
	bit = HUB_COLOR_BIT0;
	k = (period2Max - period2Min) / (HUB_COLOR_BIT0 - 1);
	for (int i = HUB_COLOR_I0; i >= 0; i--) {
		p2 = k * (bit - 1) + period2Min;
		prescaler2N[i] = p2 / period2Min;
		p1 = P1_KOEFF(p2) / HUB_PRESCALER;
		if (p1 < HUB_MIN_PERIOD) p1 = HUB_MIN_PERIOD;
		period1N[i] = p1;
		sum += p1;
		bit >>= 1;
	}
	periodDark = FRAME_PERIOD - HUB_ROWS * (period0 * HUB_COLOR_DEPTH + sum);

	HUB_TIMER2->CCR1 = period2Min;
}

static void calculatePeriod(void) {
	period0 = PERIOD0;
	if (period0 < HUB_MIN_PERIOD) period0 = HUB_MIN_PERIOD;
	setBrightness();
}	

uint16_t hubLUT[HUB_LUT_NUM] = { 0 };
	
static float gamma(float br) {
	float res = 0.0f;
	if (br < 0.0f) br = 0.0f;
	else if (br > 1.0f) br = 1.0f;
	res = (float)pow(br, HUB_GAMMA);
	if (res < 0.0f) res = 0.0f;
	else if (res > 1.0f) res = 1.0f;
	return res;
}
void hubLUTInit(void) {
	float y;
	for (int i = 0; i < HUB_LUT_NUM; i++) {
		//hubLUT[i] = i; // 256-colors
		y = (HUB_COLOR_NUM - 1.5) * gamma((float)i / (HUB_LUT_NUM - 1)) + 0.5;
		hubLUT[i] = (int)y;
	}
}

void hubScreenTest(void) {
	color_t pixel;
	int b = 255;
	int c;
	for (int y = 0; y < SCREEN_H; y++) {
		c = (y >> 2) & 7;
		for (int x = 0; x < SCREEN_W; x++) {
			b = (63 - x) << 2;
			switch (c) {
				case 0:
					pixel.R = b;
					pixel.G = 0;
					pixel.B = 0;
					break;
				case 1:
					pixel.R = b;
					pixel.G = b >> 1;
					pixel.B = 0;
					break;
				case 2:
					pixel.R = b;
					pixel.G = b;
					pixel.B = 0;
					break;
				case 3:
					pixel.R = 0;
					pixel.G = b;
					pixel.B = 0;
					break;
				case 4:
					pixel.R = 0;
					pixel.G = b;
					pixel.B = b;
					break;
				case 5:
					pixel.R = 0;
					pixel.G = 0;
					pixel.B = b;
					break;
				case 6:
					pixel.R = b;
					pixel.G = 0;
					pixel.B = b;
					break;
				case 7:
					pixel.R = b;
					pixel.G = b;
					pixel.B = b;
					break;
				default:
					pixel.ARGB = 0;
					break;
			}
			SCREEN[y][x] = pixel;
		}
	}
}	

void hubGammaTest(void) {
	color_t pixel;
	int b = 255;
	for (int y = 0; y < SCREEN_H; y++) {
		for (int x = 0; x < (SCREEN_W >> 1); x++) {
			if ((y >= (SCREEN_H >> 2)) && (y < 3 * (SCREEN_H >> 2)) &&
					(x >= (SCREEN_H >> 2)) && (x < 3 * (SCREEN_H >> 2))) {
					pixel.R = b >> 1;
					pixel.G = b >> 1;
					pixel.B = b >> 1;

			} else { 
				if (((x & 1) ^ (y & 1)) == 0) {
					pixel.R = b;
					pixel.G = b;
					pixel.B = b;
				} else {
					pixel.R = 0;
					pixel.G = 0;
					pixel.B = 0;
				}
			}
			SCREEN[y][x] = pixel;
		}
	}
	for (int y = 0; y < SCREEN_H; y++) {
		for (int x = (SCREEN_W >> 1); x < SCREEN_W; x++) {
			if ((y >= (SCREEN_H >> 2)) && (y < 3 * (SCREEN_H >> 2)) &&
					(x >= SCREEN_H + (SCREEN_H >> 2)) && (x < SCREEN_H + 3 * (SCREEN_H >> 2))) {
					pixel.R = b >> 1;
					pixel.G = 0;
					pixel.B = 0;

			} else { 
				if (((x & 1) ^ (y & 1)) == 0) {
					pixel.R = b;
					pixel.G = 0;
					pixel.B = 0;
				} else {
					pixel.R = 0;
					pixel.G = 0;
					pixel.B = 0;
				}
			}
			SCREEN[y][x] = pixel;
		}
	}
}	

void hubGammaTestGB(void) {
	color_t pixel;
	int b = 255;
	for (int y = 0; y < SCREEN_H; y++) {
		for (int x = 0; x < (SCREEN_W >> 1); x++) {
			if ((y >= (SCREEN_H >> 2)) && (y < 3 * (SCREEN_H >> 2)) &&
					(x >= (SCREEN_H >> 2)) && (x < 3 * (SCREEN_H >> 2))) {
					pixel.R = 0;
					pixel.G = b >> 1;
					pixel.B = 0;

			} else { 
				if (((x & 1) ^ (y & 1)) == 0) {
					pixel.R = 0;
					pixel.G = b;
					pixel.B = 0;
				} else {
					pixel.R = 0;
					pixel.G = 0;
					pixel.B = 0;
				}
			}
			SCREEN[y][x] = pixel;
		}
	}
	for (int y = 0; y < SCREEN_H; y++) {
		for (int x = (SCREEN_W >> 1); x < SCREEN_W; x++) {
			if ((y >= (SCREEN_H >> 2)) && (y < 3 * (SCREEN_H >> 2)) &&
					(x >= SCREEN_H + (SCREEN_H >> 2)) && (x < SCREEN_H + 3 * (SCREEN_H >> 2))) {
					pixel.R = 0;
					pixel.G = 0;
					pixel.B = b >> 1;

			} else { 
				if (((x & 1) ^ (y & 1)) == 0) {
					pixel.R = 0;
					pixel.G = 0;
					pixel.B = b;
				} else {
					pixel.R = 0;
					pixel.G = 0;
					pixel.B = 0;
				}
			}
			SCREEN[y][x] = pixel;
		}
	}
}	

const uint32_t testBmp[SCREEN_H * SCREEN_W] = { 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x607d54, 0x61794f, 0x63754f, 0x566940, 0x365024, 0x143106, 0x12d00, 0x4406, 0x5511, 0x186a23, 0x588946, 0x75a25e, 0x6aa35e, 0x3d7f38, 0x1d5d0b, 0x244c01, 0x294903, 0x3a5c0d, 0x38630b, 0x2d5202, 0x334409, 0x2f4414, 0x3b5c30, 0x4b734a, 0x59805b, 0x678565, 0x56774e, 0x6e5c34, 0x973823, 0xa22208, 0x973300, 0x785008, 0x646e28, 0x666928, 0x6b4418, 0x744b10, 0x687400, 0x498c00, 0x2d9c00, 0x2f9500, 0x427a00, 0x416c00, 0x316400, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x517447, 0x5c724e, 0x5f714c, 0x4b5f36, 0x244110, 0x2700, 0x2100, 0x2c00, 0x3800, 0x5200, 0x2b731a, 0x4d803b, 0x4f8741, 0x3a7e2f, 0x2c6414, 0x284a02, 0x253d00, 0x264900, 0x384e00, 0x574500, 0x663e0e, 0x5e3b18, 0x5a5030, 0x5c6c45, 0x5a7a54, 0x55815a, 0x4a7749, 0x5d6437, 0x7a4c25, 0x853c06, 0x743800, 0x574000, 0x5b5d00, 0x6d7c3e, 0x747b6a, 0x696a5f, 0x4e6a39, 0x348900, 0x2c9100, 0x328000, 0x397100, 0x3c6a00, 0x306900, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x48693d, 0x536a47, 0x516641, 0x3f5529, 0x243f0d, 0x73000, 0x2c00, 0x3500, 0x3d00, 0x4e00, 0x226604, 0x3c7524, 0x53843c, 0x4b823a, 0x31641c, 0x2a4305, 0x223400, 0x1f3b00, 0x584200, 0x994310, 0xad3f1b, 0xa52e16, 0x92321c, 0x825235, 0x5e643a, 0x326735, 0x3c683b, 0x3d5928, 0x383600, 0x543f00, 0x78682c, 0x847c5d, 0x827c6b, 0x696c67, 0x505e50, 0x405b34, 0x326518, 0x1f7900, 0x147800, 0x216d00, 0x2e6204, 0x326116, 0x3a7222, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x436130, 0x4b6438, 0x465e36, 0x3e542a, 0x2e4917, 0x203f04, 0x114100, 0x124b00, 0x285303, 0x305c0c, 0x3e6a1f, 0x537a38, 0x64844c, 0x587c3f, 0x3f6223, 0x294308, 0x143100, 0x2c3402, 0x823700, 0xcf2e00, 0xe53717, 0xd13c27, 0xc42913, 0xaf2003, 0x6e2c01, 0x1d3700, 0x3c03, 0x335522, 0x74866c, 0x767973, 0x676769, 0x5c5c55, 0x3a3f2c, 0x152702, 0x1b4400, 0x2e6b00, 0x2b6c00, 0x1c5e03, 0x135905, 0x19570b, 0x275a19, 0x336229, 0x436e36, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x3a551f, 0x415728, 0x3b5129, 0x3b4e28, 0x3b5021, 0x305110, 0x2c5907, 0x366110, 0x4d6c29, 0x526d32, 0x506d36, 0x5c7842, 0x657d4b, 0x5f7944, 0x4a6630, 0x2a4611, 0xa3401, 0x2e1900, 0x8b1e00, 0xad6355, 0x869492, 0x79b0a7, 0x90af9e, 0xa6987f, 0x908e6e, 0x528558, 0x5f8167, 0x888f9d, 0x767888, 0x3a3c38, 0x92200, 0x52a00, 0x153000, 0x1d350f, 0x21450a, 0x1e5103, 0x1c5109, 0x1a4c11, 0x184915, 0x1d4e18, 0x215719, 0x2b5c26, 0x3a5c33, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2e4e12, 0x324b17, 0x2c4618, 0x2f441b, 0x394e1c, 0x365610, 0x365c0a, 0x496b21, 0x5c7b3c, 0x5c7642, 0x577042, 0x5b7444, 0x5f7747, 0x5e7849, 0x4b6838, 0x294d1a, 0x181c00, 0x402a01, 0x62988d, 0xddcc, 0xc8a3, 0xaf83, 0xb384, 0xa88b, 0x2da28c, 0x568572, 0x404b51, 0x41272c, 0x340b00, 0x3b3200, 0x304f0f, 0x11450c, 0x33606, 0x112e0e, 0x153610, 0x163f0c, 0x1a4010, 0x1f3b18, 0x19411a, 0x144c17, 0x105012, 0x174715, 0x224320, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x284a0e, 0x24410d, 0x213e0f, 0x203c0e, 0x263c09, 0x2c4a00, 0x315a03, 0x436b21, 0x4f7835, 0x54733f, 0x576d42, 0x546e42, 0x577146, 0x547348, 0x43693f, 0x353d05, 0x375232, 0xd0c1, 0xf2be, 0x2aba53, 0x2a7e3d, 0x2d613e, 0x7400c, 0x1b07, 0x1327, 0x20525, 0x180a02, 0x762d01, 0x812c13, 0x674916, 0x406018, 0x17560d, 0x54105, 0x11330e, 0x183313, 0x1b3516, 0x1e3618, 0x1e361a, 0x1d401a, 0x1a4715, 0x124611, 0xf3710, 0x123012, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x224913, 0x213e12, 0x153206, 0x122b01, 0x142800, 0x1e3c00, 0x2c5905, 0x316415, 0x326722, 0x436b36, 0x536d41, 0x546c42, 0x516b43, 0x4c7146, 0x4e6230, 0x4a6b49, 0xd3bb, 0xe099, 0xb451, 0x16883c, 0x413934, 0x260616, 0xb0e07, 0x10e22, 0x1941, 0x112e, 0x643b16, 0x9a3807, 0x842408, 0x6b4515, 0x415f18, 0x1c5a0e, 0x144710, 0x183815, 0x173516, 0x1a3617, 0x1e361a, 0x1a3417, 0x143f15, 0x174a10, 0xd430b, 0xf330e, 0x162d14, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x25471d, 0x173c11, 0xd3203, 0x133200, 0x1a3400, 0x1e3901, 0x1a4a05, 0x195a13, 0x2d6629, 0x406d3b, 0x4e6d42, 0x556c43, 0x4b6a40, 0x4e6b3d, 0x5d6425, 0xb895, 0xe695, 0xa663, 0x7b78, 0x6765, 0x223944, 0x61f26, 0x3c70, 0x2869, 0x120e2a, 0x161619, 0x5a440e, 0x703300, 0x6b3101, 0x4f4407, 0x2c5109, 0x1d5010, 0x1e4916, 0x1b3f17, 0x1b3717, 0x1b3718, 0x1e391a, 0x1b3717, 0x123e0f, 0xa4b06, 0x34a06, 0x73c09, 0x143111, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x234220, 0x123910, 0x1b3f07, 0x244103, 0x2a4603, 0x2f4d12, 0x28511e, 0x28612b, 0x366a37, 0x3a6838, 0x43693c, 0x527146, 0x49703f, 0x426529, 0x435e30, 0x8c87, 0x8484, 0x5280, 0x4299, 0x131e61, 0x31f4d, 0x92256, 0x1c62, 0x3656, 0x2d34, 0x1e3e38, 0x4c6b23, 0x59661d, 0x646e32, 0x4d6c27, 0x1d5d0d, 0xe4b04, 0x1b440e, 0x1b4116, 0x193915, 0x1c3918, 0x1f3b1c, 0x1d3b19, 0x12420b, 0x65207, 0xf5609, 0x11450a, 0x103610, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x1a3818, 0x193d11, 0x2f4b0d, 0x384e0a, 0x3f5317, 0x425a29, 0x3f5a2e, 0x39622c, 0x396e37, 0x477846, 0x5a8758, 0x6b9469, 0x67905c, 0x4b7d37, 0x2e60aa, 0x6447e1, 0x7c3cd6, 0x4c24da, 0x2e0686, 0x105b, 0x2855, 0x5669, 0x776c, 0x6849, 0x5d45, 0x4645, 0x668c62, 0x84b16e, 0x80ac73, 0x6ea065, 0x4a8640, 0x1f6214, 0xd4507, 0x1a3c14, 0x163b16, 0x1d3d19, 0x1c3c18, 0x163f16, 0x144713, 0x1d5617, 0x235919, 0x184615, 0x174a1d, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x143512, 0x163304, 0x2d4500, 0x495a08, 0x455a11, 0x394f18, 0x2a4a19, 0x316632, 0x5f8f5c, 0x81ad7f, 0x9cba96, 0xa5bd93, 0x9db98b, 0x92b670, 0x52948b, 0x4f9b, 0x4394, 0xe78b4, 0x7f8c, 0xbda4, 0xd7b7, 0xcd89, 0xc576, 0x16140, 0x5741, 0x383d, 0x63937a, 0xb3da9c, 0x99c699, 0x92c190, 0x77ab73, 0x4d8445, 0x1a5617, 0x133b0e, 0x153e17, 0x1d421f, 0x1d401b, 0x1a421b, 0x234a1f, 0x2c5627, 0x2e562b, 0x224b28, 0x366531, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x17350f, 0x1b3d01, 0x365606, 0x466710, 0x476b1d, 0x21530c, 0x1f550f, 0x608242, 0x91b17e, 0xb4cda9, 0xa2cd84, 0x69c239, 0x75c823, 0x86ba58, 0x428a7c, 0x8b66, 0xa669, 0xc65e, 0xf6a3, 0xffbb, 0xdf94, 0x8c5b, 0x12754c, 0x7858, 0x624a, 0x4b4c, 0x11827f, 0xaecf9b, 0x9fc799, 0xa0c79b, 0x93bf8e, 0x6d9c66, 0x306c2b, 0x14108, 0x193d18, 0x224623, 0x264625, 0x284622, 0x2f4a27, 0x33502f, 0x365133, 0x345036, 0x4e7743, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x123c00, 0x96ae74, 0xdbebcc, 0xc7dcbc, 0xcde4c9, 0xb8d7b7, 0xc0ce9f, 0xa9b35f, 0xa0b972, 0x8cc969, 0xbe00, 0xbd00, 0x46c400, 0x3c9a49, 0x1b7791, 0x41b981, 0x2ec062, 0xa96d, 0xdd96, 0xdc7e, 0xc06b, 0x9069, 0x5b47, 0x6a53, 0x685c, 0x5556, 0x6173, 0x60b998, 0xaac587, 0x8cb886, 0x96bd8b, 0x83af7a, 0x518b4f, 0x226221, 0x1a4a1a, 0x224621, 0x274f24, 0x285122, 0x2d4d25, 0x354d30, 0x3f5338, 0x3f523d, 0x557c50, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x8aac6d, 0xffffff, 0xffffff, 0xffffff, 0xffffff, 0xffffff, 0xcfdeaa, 0x68b335, 0xb200, 0xad00, 0x6abe00, 0x63b600, 0x615f, 0x1d6d6f, 0x7ba7f, 0x42f281, 0xc666, 0x963c, 0x9931, 0x833d, 0x966f, 0x76c51, 0x402c, 0x5a3f, 0x4e4d, 0x6872, 0xb58b, 0x99bd68, 0x4e985b, 0x6daa6d, 0x8ab67b, 0x7eaa74, 0x64905c, 0x3d713a, 0x14521d, 0x195314, 0x225a13, 0x1e5319, 0x2f502e, 0x455a41, 0x4b5f4a, 0x587d52, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x205426, 0x3800, 0x4f8441, 0xbde4bb, 0xfcfffa, 0xffffff, 0xffffff, 0xcdd59e, 0xf9c00, 0xa500, 0x71a900, 0x96ae00, 0x5d8d35, 0x5765, 0x7247, 0x17c269, 0x29e77a, 0xa83f, 0x8a2d, 0x861e, 0x7636, 0x472d, 0xd4127, 0x7950, 0x673c, 0x5656, 0x676f, 0x3e8e4f, 0x9d9b1f, 0x688343, 0x639456, 0x71b56f, 0x91c082, 0x8ab37e, 0x6d9768, 0x357534, 0x1c5a00, 0x2b5400, 0x2c5311, 0x35522f, 0x4b5f4b, 0x576a59, 0x627d55, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x73a77b, 0x457947, 0x305e15, 0x58823b, 0x94c084, 0xc1e8bd, 0xf2f0d5, 0xcdd087, 0x449c00, 0x51a300, 0x7db800, 0x8da900, 0x617563, 0x1b8659, 0x8a40, 0x2a7230, 0x516b36, 0x218d58, 0xba57, 0x852f, 0x6345, 0x6135, 0x5228, 0x5a2f, 0x5031, 0x503d, 0x636a, 0x157245, 0x717d00, 0xa06e28, 0x8c6b2d, 0x569d4c, 0x73b86e, 0x87be7b, 0x83b078, 0x6a9359, 0x426c12, 0x3a5000, 0x3f4912, 0x404e35, 0x4d6653, 0x617265, 0x657a5b, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0xddf9e3, 0x96cb89, 0x6b9c47, 0x6c8d44, 0x5b8633, 0x478e2f, 0x86a357, 0xb19c48, 0x6a9000, 0x5ea500, 0x86cf00, 0x679236, 0x313b3c, 0x22b160, 0x5b22, 0x1b1413, 0x44734e, 0xc67a, 0xac65, 0x724a, 0x7e5a, 0x7a49, 0x754c, 0x764d, 0x472e, 0x3630, 0x6240, 0x9203, 0x8500, 0x625b00, 0x8b3600, 0x637821, 0x399a3d, 0x5aa054, 0x76a769, 0x7ba168, 0x668640, 0x605104, 0x613010, 0x584439, 0x526959, 0x64796a, 0x60745a, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0xb4dfaa, 0x7fb165, 0x6c9945, 0x61913f, 0x46831e, 0x327e09, 0x618423, 0xb97d35, 0x958e16, 0x74b000, 0x8bbf38, 0x5f6b3a, 0x294317, 0x2d6e39, 0x241e0f, 0x423b39, 0x905d, 0xb85d, 0x8559, 0x5141, 0x6937, 0x5623, 0x692c, 0x834e, 0x6247, 0x1e34, 0x205209, 0x55cb00, 0x9d0a, 0x2f7807, 0x705a00, 0x516b10, 0x37716, 0x247620, 0x47873f, 0x689759, 0x738f52, 0x83632d, 0x8b2912, 0x7a3029, 0x625b57, 0x596971, 0x636e65, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x619651, 0x507e46, 0x348136, 0x8209, 0x8300, 0x269234, 0x6faf64, 0xc09f63, 0xa39a43, 0x77c335, 0x80ad49, 0x6c4d38, 0x59631d, 0x2a4e0e, 0x1b0604, 0x1d5f50, 0xb979, 0xa04f, 0x9a6c, 0x9f9c, 0x5a62, 0x3386, 0x368b, 0x4052, 0x1c40, 0x29, 0x3b471a, 0x7dbe00, 0x5aa500, 0x11981a, 0x9111, 0x1a7b18, 0x276b23, 0xd631c, 0x6617, 0x3c752f, 0x7c8045, 0xa45e3c, 0xb83d31, 0xaa4b3f, 0x857360, 0x5f855c, 0x9cc79d, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0xc5e7c9, 0xb0b598, 0x987246, 0x6a761e, 0x428e2a, 0x61c16e, 0x7de79d, 0xa7b374, 0xa09054, 0x7ccc62, 0x709a41, 0x5f281e, 0x5d3c25, 0x163f07, 0x110c02, 0x9869, 0xab61, 0x712d, 0xb377, 0x866e, 0x68c1, 0x39cc, 0x13ab, 0x20c95, 0x30253, 0xd3d, 0x6e8f63, 0xb6b647, 0x6e9e00, 0x9d00, 0xa207, 0x8515, 0x1c6a2b, 0x386338, 0x54b18, 0x717742, 0xb7925d, 0xb6a048, 0xabb926, 0x8ad400, 0x73ed00, 0x7af500, 0x8fed00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0xffffff, 0xfff7f3, 0xf47175, 0xef4a54, 0xf06964, 0xf9a289, 0xeabe94, 0xe68b73, 0xde7d6f, 0x9dc788, 0x629055, 0x4d1f11, 0x28140e, 0x2917, 0x171707, 0x6333, 0x6d2e, 0x5308, 0x5f23, 0x451d, 0x5674, 0x2a77, 0x1054, 0x71054, 0x44, 0x4d4c, 0xbed079, 0xcfc26f, 0x3e8700, 0x7100, 0x8000, 0x911e, 0xb6c287, 0xcbc485, 0x8eaa76, 0xc7d838, 0xa2e600, 0x5df100, 0x14fb00, 0x59ed00, 0x90c700, 0x9da700, 0xd9b602, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0xffffff, 0xdff8e8, 0x678d5c, 0x6c7143, 0xa5715f, 0xc83941, 0xe03343, 0xeb4a5d, 0xed405d, 0xf26b7b, 0x84946c, 0x21290e, 0x90303, 0x1b0e09, 0x1b2008, 0x6830, 0x672b, 0x5f18, 0x3e0c, 0x3726, 0x1b20, 0x1750, 0x1339, 0x1, 0x1b3d, 0x7aa05b, 0xdcd46f, 0xa5ab4d, 0x488300, 0x7aa94f, 0x6cbc52, 0xbee3af, 0xffffff, 0xf3ffc8, 0xdbff14, 0x8bf800, 0xe700, 0xdc00, 0x6fd400, 0xb09e00, 0xb18f6b, 0xd9d3c2, 0xe1ebaa, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0xf8fffb, 0xb9debb, 0x589b52, 0x79935c, 0x8a815d, 0x7b624d, 0x767262, 0x7b8b7c, 0x867457, 0xb95d22, 0x847635, 0x1d2310, 0x70203, 0x230504, 0x114512, 0x7f35, 0x5c24, 0xd551c, 0x53312, 0x1919, 0x20e16, 0x9090f, 0x4040e, 0x101a19, 0x808e4c, 0xc6b550, 0x989c3c, 0x456800, 0xb3cc8b, 0xffffff, 0xffffff, 0xffffff, 0xfffff8, 0xc5fb4a, 0xbbd300, 0xd09a00, 0x80a300, 0x72aa00, 0x9fcd00, 0xcde8a6, 0xe2f8ed, 0xf4fff4, 0xc9ffe0, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0xc9f6d5, 0xa8b682, 0xbc774f, 0xb36e55, 0x817558, 0x67936e, 0x5eb567, 0x54c343, 0x2ccc00, 0x54c700, 0x41643f, 0x220509, 0x100403, 0x80302, 0x300a, 0x5b1a, 0x13710, 0x52a13, 0x1e11, 0x60e0c, 0x9090c, 0x20305, 0x80e05, 0x5f873f, 0xa0b351, 0x918327, 0x69852e, 0xc5d4b0, 0xfffafe, 0xe9ffef, 0xeaffdd, 0xd1f897, 0xd79924, 0xd59d0d, 0x9ab312, 0xdd8216, 0xff462f, 0xff6950, 0xc6fbcb, 0xdffff6, 0xc8f7cb, 0x95df97, 0xaae5b0, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x9dc290, 0x9ba86c, 0xa58d62, 0x5c7045, 0x3c733a, 0x9c00, 0xcb00, 0xc000, 0x26ba00, 0x8fc663, 0x334f4e, 0x70000, 0x381b14, 0x1b1208, 0x1e1c0c, 0x1c4518, 0xf3e1b, 0xb1213, 0x2070a, 0x1090c, 0x40b0d, 0x161107, 0x1c0e08, 0x716c4f, 0xb4a256, 0x875e02, 0xafaf87, 0xffffff, 0xecfff6, 0xcaffe0, 0xf5ffcb, 0xc39b52, 0xc52a26, 0xff365f, 0xcd8d5a, 0xdda83e, 0xe68b51, 0xaac596, 0x92ecaa, 0x86dc8b, 0x91e19a, 0xb5ebbc, 0xc2f2bd, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x7eab6b, 0x75c188, 0x86c092, 0x989d7a, 0xbbbc85, 0xc4d35c, 0xbec345, 0xd5e379, 0xededac, 0xffe2c9, 0xeac2a3, 0xe39871, 0xdd7b49, 0xbb5a24, 0xbe541a, 0x613726, 0x1a2822, 0x37200d, 0x321002, 0x431a03, 0x491b01, 0x833504, 0x481304, 0x42272f, 0xffc4a0, 0xffcfa2, 0xffe6c1, 0xfffff3, 0xe8ffd9, 0xecfff9, 0xdcfbdc, 0xe28457, 0xff763f, 0xcf9c00, 0xa4e500, 0xa4ce00, 0x93c57f, 0x5fd989, 0x61bf51, 0x7ed585, 0xade4ae, 0xcdfcc8, 0xc7f4b6, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0xe6ffe3, 0xf8f2d5, 0xffe5c6, 0xffffdd, 0xf7b9a1, 0xb4504c, 0xbd3834, 0xfa7f70, 0xf88d75, 0xd37134, 0xd78842, 0xd0883e, 0xa35608, 0x9c5000, 0x954c00, 0x48230d, 0x130609, 0x330e01, 0x4f1f01, 0x3e1101, 0x481401, 0x532102, 0x0, 0x210d10, 0xa97054, 0x977958, 0xa28443, 0xbf8745, 0xe98852, 0xf9ac79, 0xffcfaa, 0xffe7c5, 0xfff7c3, 0xffffc6, 0xfff580, 0xdae171, 0xabdca8, 0x5bc761, 0x55d375, 0xaef3c4, 0xd0f9cd, 0xc4fab2, 0x9dca84, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0xffc8bd, 0xe68364, 0xce6b2d, 0xbd6e3f, 0x6e190e, 0x60000, 0x360500, 0x5e2000, 0x6b3300, 0x260600, 0x90100, 0x241803, 0x191c03, 0x51601, 0x51603, 0x40402, 0x40301, 0x40402, 0x0, 0x321919, 0xfda277, 0xc77c52, 0x805539, 0x6c3f22, 0x7c1c05, 0x790000, 0x8f0000, 0x7b3813, 0x606937, 0x754200, 0x825000, 0x935716, 0xa25325, 0xc86e65, 0xfc937e, 0xffc094, 0xffeeb8, 0xfff3b4, 0xe0e9a6, 0xd5dfa8, 0xd6ffc6, 0xb0eba8, 0x619e5a, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x7e5700, 0x504400, 0x3b3600, 0x717620, 0xa2be7a, 0xaacea3, 0xa5d6a1, 0xb0aa3f, 0x665f2b, 0x2, 0x20002, 0x1b170f, 0x31107, 0x10a04, 0x10303, 0x10102, 0x30402, 0x40204, 0x0, 0x867721, 0xffde7e, 0xff9b66, 0xd87b4b, 0xc56c38, 0xe93f1b, 0xff0510, 0xff0005, 0xff3e30, 0x9bd493, 0x53cc7f, 0x649d00, 0x99600, 0x398500, 0x565400, 0x583c00, 0x784c1f, 0x9b5424, 0xb44d2a, 0xf17f5c, 0xff9a7d, 0xf8bfa1, 0xcdad79, 0xc0be8c, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x93ff00, 0x7ac700, 0x437b5a, 0x649c47, 0x83c14c, 0xb9ea86, 0xedffd5, 0xcadcae, 0xd0404, 0x0, 0x1b1308, 0x231c0f, 0x30402, 0x30302, 0x10101, 0x30302, 0xa0505, 0x90009, 0x334f00, 0x1aa500, 0x3d9300, 0x859a0c, 0xb37c1b, 0xd66b31, 0xde693c, 0xff4f37, 0xff0c17, 0xff0606, 0x91a700, 0x4acb24, 0x9db32e, 0x3fcc00, 0x67d200, 0x6ec30d, 0x4ba556, 0x2b9d47, 0x347f3d, 0x5b6733, 0x58410b, 0x570c00, 0x340000, 0x3b0000, 0xe57e61, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0xfeffe2, 0xd4ebaa, 0x819b74, 0x6e9266, 0x6a8864, 0x5a733e, 0x96ae4f, 0x5c7254, 0x0, 0x0, 0x281f10, 0x141109, 0x0, 0x20202, 0x50403, 0x130b0a, 0x17040e, 0x2d3f00, 0x9b000, 0xa200, 0x9c00, 0xb000, 0x52b800, 0x9d9b00, 0xae710c, 0xd2763a, 0xff744f, 0xff5938, 0x7fab00, 0x2cd900, 0x92ad00, 0x69b300, 0x63c600, 0x76b670, 0x7abf79, 0x47bd53, 0x77d482, 0xa4fca8, 0x99e990, 0x505e30, 0x38341a, 0x3a331e, 0x531801, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00, 0x2d00 };
//const uint32_t testBmp[SCREEN_H * SCREEN_W] = {	0x15181a, 0x10207, 0x1d2229, 0x31333a, 0x2a262f, 0x292634, 0x383d43, 0x383e40, 0x141416, 0x50506, 0x1e1e26, 0x25232a, 0x272728, 0x232421, 0x202423, 0x252326, 0x322c2c, 0xb9b4bb, 0xeae8f0, 0xd9d6dd, 0xdadadf, 0xe8e1e6, 0xede2e9, 0xebe3e5, 0xe9e4e4, 0xece4ea, 0xe3dee0, 0xd6c5c3, 0xf8e1d5, 0xffefdf, 0xffe9d9, 0xfeefdd, 0xffeadd, 0xffe9dd, 0xfff5eb, 0xfffcee, 0xfffae8, 0xfffaed, 0xfffcf1, 0xfffff4, 0xfffff5, 0xfffbed, 0xfffffc, 0xfffff9, 0xfff8ed, 0xffede2, 0xfeeedf, 0xffefe1, 0xffe9dc, 0xfdebda, 0xfdedde, 0xfeecdf, 0xfeeadd, 0xffece1, 0xffede2, 0xffeadf, 0xfdebdd, 0xffede0, 0xffeadd, 0xffebdc, 0xffeee1, 0xffefe4, 0xfeeee5, 0xfeede5, 0xb0a0d, 0x1d1822, 0x33363d, 0x262b32, 0x25222a, 0x313039, 0x283039, 0xc131a, 0xf0d0d, 0x28282a, 0x323234, 0x21221d, 0x1d2220, 0x1e1e21, 0x242227, 0x1b191e, 0x221918, 0xc1b4bd, 0xeae9f2, 0xd9d6db, 0xd5d8da, 0xe6dfe2, 0xeee2e8, 0xf1e8eb, 0xfaf4f6, 0xdfeaeb, 0xdcdde0, 0xd9ccce, 0xf3e7d7, 0xfffded, 0xfbeedd, 0xfcf2df, 0xfffff7, 0xfffff4, 0xffefe0, 0xfce4d9, 0xffeddd, 0xffead4, 0xcab5a1, 0xa69082, 0xaf937b, 0xb79a7f, 0xb09680, 0xc3ab8c, 0xf5dec5, 0xffffef, 0xfffff4, 0xfffdf5, 0xfffffa, 0xfffff6, 0xfff4e7, 0xfcecdc, 0xfeeade, 0xffebde, 0xffeadb, 0xffe9dc, 0xfceadc, 0xfeeee3, 0xffeee5, 0xfeecdd, 0xffeddf, 0xffece0, 0xfceadf, 0xfeece1, 0x1f2222, 0x2b3038, 0x283035, 0x1d222a, 0x2c2a32, 0x353434, 0x242126, 0x9090f, 0x27242d, 0x33323c, 0x2f2c34, 0x2e2e2d, 0x313135, 0x2d2a35, 0x303232, 0x1b2722, 0x282322, 0xd0c3cb, 0xf1eff7, 0xe2e2e5, 0xdddddd, 0xe0dbdc, 0xfbf0f5, 0xefebf1, 0xc9ccc9, 0xb09f97, 0xbc8b8a, 0xd69ea4, 0xe9c1b7, 0xedc3bd, 0xfab9b7, 0xd2928e, 0x937560, 0x99816f, 0x514634, 0x2c2014, 0x48311c, 0x503922, 0x3e200b, 0x160200, 0x0, 0x0, 0x0, 0x0, 0x3a270f, 0x8c7f5c, 0xc0b08c, 0xd1c0a6, 0xd7c2b1, 0xecd6bf, 0xfff8e2, 0xffffff, 0xffede4, 0xffe9da, 0xfceadc, 0xfdece1, 0xffefe2, 0xfeece1, 0xffebe2, 0xffece2, 0xfeeadd, 0xfee9db, 0xfeedde, 0xfeede1, 0x2b4043, 0x183637, 0x1f262f, 0x2b2431, 0x2e2b35, 0x2a2a31, 0xe0e17, 0x131b1a, 0x323439, 0x2b272f, 0x232122, 0x2f2c30, 0x2a2931, 0x2c2932, 0x2c3031, 0x212b29, 0x514a4c, 0xebe4ed, 0xfbfeff, 0xe8ebf0, 0xdddde4, 0xf3eef4, 0xc5c7c6, 0xaa8181, 0xc28984, 0xbf8273, 0xd58684, 0xde8c92, 0xf3a3a7, 0xe78f97, 0xbd6d6b, 0x793f38, 0x170000, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x1d0400, 0x180601, 0x0, 0x0, 0x0, 0x0, 0x40000, 0x311400, 0x421e06, 0x240c02, 0x140700, 0x443523, 0xc3ac92, 0xfffff9, 0xffeee3, 0xfbebda, 0xfbede0, 0xffefe0, 0xfeecdc, 0xfde7da, 0xffece4, 0xfeede0, 0xffecde, 0xffeddf, 0xfeecde, 0x2d3944, 0x262c33, 0x242932, 0x232831, 0x2f2d38, 0x232029, 0xb0e19, 0x252a38, 0x2e2c33, 0x212120, 0x16221b, 0x1a1e1c, 0x171715, 0x1a1a1e, 0x131320, 0x131017, 0x3c3233, 0xaba9a9, 0xc5c7c6, 0xd0cdd0, 0xf0eff6, 0xffffff, 0xb6c5c9, 0x714742, 0xe38d89, 0xbf8e83, 0xad897c, 0xaa847a, 0xa18377, 0x9e7868, 0x826257, 0x6e5b50, 0x745c57, 0x4f443c, 0x373728, 0x2d261b, 0x271718, 0x1b0e10, 0x180a03, 0x210d00, 0x3e2416, 0x462d25, 0x36231f, 0x2f2421, 0xd0a04, 0x190500, 0x391004, 0x300b05, 0x30000, 0x0, 0x0, 0x0, 0xa99b6c, 0xfffff4, 0xfff1e6, 0xffebdd, 0xffe6db, 0xfee9da, 0xffeadb, 0xfeecde, 0xffeadf, 0xffebe0, 0xffebde, 0xfdecdd, 0x3d3d46, 0x312e38, 0x24222b, 0x262b32, 0x2e3331, 0x141211, 0x252324, 0x312e39, 0x161822, 0x14161a, 0xc1c16, 0x6120c, 0x50505, 0x40404, 0xa0c0d, 0x2b2427, 0x3b3533, 0x32312b, 0x2b3025, 0x31342e, 0x726d74, 0xc4bec2, 0xb7b1ad, 0x816963, 0x8f6a61, 0x8c7268, 0x876d63, 0x8c6d64, 0x8d6d68, 0x9a7870, 0xb2857b, 0xba8c83, 0xb99087, 0xbe948c, 0xb49480, 0x9e7b6e, 0x87625c, 0x865e5a, 0x855c55, 0x7b524d, 0x7e524f, 0x896159, 0x996d65, 0x9a7d6e, 0x7a6d5c, 0x42362b, 0x441e19, 0x2a0e04, 0xa0400, 0xc0403, 0x10000, 0x0, 0x1f1600, 0xfff4db, 0xfffffa, 0xffeedf, 0xfeecde, 0xffede0, 0xffeee1, 0xfeede0, 0xfee9de, 0xfce8dc, 0xfde9dc, 0xfdebdf, 0x46444c, 0x313038, 0x2f2d35, 0x36363d, 0x252727, 0xf1110, 0x282b29, 0x1d1a19, 0xd0d16, 0xb0b1b, 0x1c1c22, 0x1d201d, 0xf090b, 0x100e0e, 0x232322, 0x3d3f3e, 0x3d3f40, 0x27272a, 0x141c1e, 0x1d261e, 0x594546, 0x936c64, 0xae8070, 0xc29387, 0xb69182, 0xb3867e, 0xb7867e, 0xbb897d, 0xc28c7f, 0xcd9287, 0xd89a8b, 0xdd9c8f, 0xd1998c, 0xc39178, 0xcd886f, 0xbf8777, 0x875e5a, 0x8f5e58, 0xb17a71, 0xc68980, 0xb87f71, 0xb47c6f, 0xb37f72, 0xae8470, 0xb68978, 0x9b756f, 0x634649, 0x442a26, 0x5c463c, 0x3c2a22, 0x0, 0x0, 0x30000, 0xcda888, 0xffffff, 0xfcefe2, 0xfceee2, 0xfeece2, 0xffeadf, 0xffebde, 0xfdecdb, 0xfae8d8, 0xfcebdd, 0xffefe5, 0x3e3e45, 0x302e35, 0x34353b, 0x302e36, 0x17161f, 0x212028, 0x2d2d34, 0x110d15, 0x12171f, 0x19242d, 0x191a22, 0x191116, 0x261f21, 0x262827, 0x1f1e2c, 0x23242e, 0x2d2e2b, 0x202a22, 0x514444, 0x9b7973, 0xc29d89, 0xd1a18a, 0xd1a093, 0xcda295, 0xc89d91, 0xc8928c, 0xcd9789, 0xdc9f92, 0xe6a195, 0xde998a, 0xe0968a, 0xe79c90, 0xd89c8a, 0xc28674, 0x9a5041, 0xb27360, 0xd09995, 0xb87c78, 0xc08577, 0xc78a7e, 0xcf9785, 0xce9287, 0xcf8d86, 0xc68f7f, 0xc08879, 0xc7857f, 0xa9767a, 0x624946, 0x624942, 0x26100b, 0x0, 0x40200, 0x40100, 0x472805, 0xefd8c0, 0xfffef9, 0xffece3, 0xfee9e0, 0xffebdd, 0xffebdc, 0xffe6d9, 0xffe6d9, 0xfdefde, 0xfcf1e3, 0x39393f, 0x3b3b40, 0x35343d, 0x302b33, 0x161619, 0x201c26, 0x24212b, 0x14121d, 0x1b1f27, 0x1f302a, 0x161b19, 0x262329, 0x1e282e, 0x2f2c31, 0x63494d, 0x71514a, 0x4d3f38, 0x735e53, 0xb99084, 0xd0a28d, 0xc7a58a, 0xc6a18e, 0xc6998a, 0xbc897a, 0xd9a299, 0xf3bbac, 0xebb1a2, 0xf3a9a1, 0xfeaca9, 0xfaaba4, 0xf3a09c, 0xed9891, 0xde8f7c, 0xda8979, 0xb26a6a, 0x986767, 0xecaaa3, 0xda9a95, 0xbb8373, 0xc6867d, 0xde9d92, 0xe6a99a, 0xe59f93, 0xdea08d, 0xd5998b, 0xd6938b, 0xca8989, 0x6d4747, 0x311108, 0x31170e, 0x230907, 0x311f14, 0x30201, 0x0, 0x958155, 0xfffff7, 0xffece2, 0xffe7da, 0xffece0, 0xffece0, 0xffeadf, 0xffece4, 0xffefe4, 0xfcefe3, 0x393941, 0x33303e, 0x2e3138, 0x28282f, 0x19181f, 0x191e25, 0x171920, 0x1f1a25, 0x1e1e20, 0x161c1a, 0x21211f, 0x19232a, 0x504351, 0xc58f87, 0xb78c77, 0x725d46, 0x71534f, 0xbb8980, 0xcc9d88, 0xcd9e88, 0xd6aa98, 0xbd998d, 0x765d4b, 0x5a3928, 0x7a5344, 0xcd8c79, 0xffb6a7, 0xffb8b2, 0xffb1ac, 0xffb2b1, 0xffacac, 0xf19690, 0xe3897a, 0xe8917d, 0xd49393, 0x6f5d63, 0xc38375, 0xfab7b4, 0xab776d, 0xd19483, 0xe8a398, 0xeda899, 0xeba591, 0xefa595, 0xeda399, 0xe4a194, 0xc78c7b, 0xb67d76, 0x5c2e2d, 0x492020, 0x694d4c, 0x412e26, 0x60003, 0x0, 0x4a3d14, 0xfff7e0, 0xfff5ee, 0xfeeee1, 0xffece2, 0xffe8de, 0xfdebdd, 0xfff0e7, 0xfff0ea, 0xffefe5, 0x423e49, 0x343342, 0x31313d, 0x171a26, 0xf141c, 0x1a1f28, 0x13101f, 0x231f28, 0x1f1e23, 0x19161e, 0x161f23, 0x3b3b3b, 0xcb958e, 0xd9a28d, 0x775d52, 0x584441, 0x93786b, 0xb88f80, 0xcd9988, 0xd29e8b, 0xd6a393, 0xbd9281, 0x61423a, 0x52352d, 0x624739, 0x5c2d1f, 0xcf7861, 0xffc8bd, 0xffbec4, 0xffb1b3, 0xffaaab, 0xfc9c98, 0xeb897f, 0xf8a095, 0xca9b9a, 0x534454, 0xb77e74, 0xf9b39f, 0x99625f, 0xcd9080, 0xfab3a8, 0xefaa9d, 0xeea898, 0xf4a499, 0xf5a799, 0xf0aa97, 0xdb9d87, 0xcd8776, 0x8f5752, 0x4c2922, 0x6c554e, 0x423333, 0xa0404, 0x0, 0x1f1400, 0xd9c69f, 0xffffff, 0xffeee4, 0xffebe1, 0xffecde, 0xffecde, 0xffecde, 0xfff0e8, 0xffeee5, 0x434044, 0x353541, 0x362e4b, 0x1d1b2f, 0xa1015, 0x1e1b27, 0xe071c, 0x12161f, 0x231f29, 0x231721, 0x201f22, 0xb1887d, 0xf0b39f, 0xa0756e, 0x5f4646, 0x6f5a51, 0x967d6e, 0xbb9585, 0xcfa18e, 0xcc9b89, 0xc3907b, 0xd89b85, 0xcf9281, 0x84554e, 0x6b4c3a, 0x4c2b18, 0x803b29, 0xffa08b, 0xffcbc6, 0xffbcc0, 0xffa3a6, 0xff9e9d, 0xf68f8f, 0xe7908a, 0xebacac, 0xc8b0b4, 0xd2928e, 0xe08e78, 0xa45f5c, 0xd29182, 0xffbcb0, 0xf9aea5, 0xefa99c, 0xf7afa1, 0xf8aba2, 0xf6a99d, 0xf8b19f, 0xd89085, 0xa0625a, 0x5b3530, 0x5d3936, 0x483e36, 0x302a24, 0x40000, 0x270701, 0x724d2b, 0xf4e5cf, 0xfff7f2, 0xffebdf, 0xffefe3, 0xffede2, 0xffe8db, 0xfeeddf, 0xffede4, 0x37373e, 0x252330, 0x2d293d, 0x191822, 0x121114, 0x1d1a25, 0xf0d17, 0x15181d, 0x19191b, 0x111416, 0x614945, 0xe3a095, 0xd19b89, 0x886764, 0x4c4642, 0x78675d, 0xb48f7a, 0xcd9f8a, 0xd2a291, 0xcb9c89, 0xc78e79, 0xd39280, 0xeca79b, 0xae6c6a, 0x6f4331, 0x56280f, 0x5c1f1a, 0xb25a4b, 0xffa797, 0xffc9c5, 0xffb5b3, 0xff9f9b, 0xfe9c9a, 0xe98e8b, 0xe18687, 0xea9b93, 0xdb8e7d, 0xcd8573, 0xc38177, 0xe4a292, 0xffbcb2, 0xfdada4, 0xf5a8a0, 0xfab1a4, 0xfcb1a5, 0xf8aba1, 0xf5aa9e, 0xf2a999, 0xc47f79, 0x481b25, 0x53382f, 0x6f5e50, 0x635450, 0x29160e, 0x340d07, 0x2d0a02, 0xb6a57a, 0xfffffd, 0xfdeddf, 0xffebe1, 0xffece2, 0xffeadb, 0xffecdc, 0xffeee1, 0x383e4b, 0x252d34, 0x252a2d, 0x10131d, 0x1d1a1f, 0x222228, 0x111114, 0x11110b, 0x171a18, 0x101926, 0x806061, 0xdca38f, 0xc09280, 0x755f57, 0x595044, 0x9f8175, 0xcea597, 0xd1a297, 0xd39c95, 0xdba297, 0xd59583, 0xd19386, 0xdb9e93, 0xbf7171, 0x8a4443, 0x57261c, 0x300701, 0x440905, 0xc6655d, 0xf79d93, 0xe7928e, 0xeb8b81, 0xeb9386, 0xe6928a, 0xd68174, 0xd98476, 0xd58475, 0xdb9185, 0xecb3ac, 0xffc0bb, 0xffb5ad, 0xfba49d, 0xfaaaa2, 0xfdb1a6, 0xfbb1a3, 0xf6ae9e, 0xf2a89b, 0xf5a99a, 0xd9988e, 0x4b281f, 0x2b0901, 0x6a4d44, 0x746a62, 0x553d38, 0x340c05, 0x290500, 0x977a51, 0xfffcec, 0xffefe2, 0xffeee0, 0xfff2e5, 0xffefe2, 0xffede1, 0xffede2, 0x373c42, 0x35353d, 0x2a2e32, 0x131a27, 0x15171b, 0x1f2629, 0xb0e17, 0xa0907, 0x222228, 0x121620, 0x7a5357, 0xe9b1a1, 0xcba090, 0x6d5752, 0x745b50, 0xb69081, 0xd2aca5, 0xd6aea9, 0xd3a3a0, 0xdca199, 0xd69483, 0xd7958c, 0xeca4a0, 0xb66366, 0x682a28, 0x431c15, 0x2a1102, 0xa0000, 0x792527, 0xd0726b, 0xbb726a, 0xb56863, 0xc97b6b, 0xcd8174, 0xc67c71, 0xc77f72, 0xe2927d, 0xf4a994, 0xffccc1, 0xffd1c8, 0xffada1, 0xf7a19a, 0xf9b1a6, 0xfcb4a7, 0xfdaea1, 0xf9ad9d, 0xf3ac9b, 0xf6aa9c, 0xe59d93, 0x6a3f38, 0x110200, 0x280c03, 0x634f49, 0x695152, 0x401a0d, 0x2e0403, 0x87624c, 0xfff8e5, 0xffeee5, 0xffece3, 0xffefe0, 0xfff2e5, 0xfeefe5, 0xffeee2, 0x3d3c42, 0x302d36, 0x282e31, 0x142928, 0xc1417, 0x181b20, 0x11111d, 0x60612, 0x1f1f29, 0xb1812, 0x84635b, 0xffbdbc, 0xd39f95, 0x755b55, 0x745a56, 0xaf8b83, 0xdbb1ab, 0xe0b9b5, 0xe2aaac, 0xc88282, 0xc67f74, 0xde9993, 0xf8a3a8, 0xc7636f, 0x73362b, 0x53261c, 0x2b0901, 0x260d02, 0x683927, 0xd08077, 0xeb9690, 0xca7770, 0xcd7575, 0xd58279, 0xd6857e, 0xd9877f, 0xf6a091, 0xffbaa5, 0xffd0c3, 0xffc3b7, 0xfda99f, 0xffaaa3, 0xffb3a8, 0xfcb1a4, 0xfeae9f, 0xfcab9d, 0xf7af9d, 0xfab09f, 0xe79c9a, 0x7c5451, 0x20000, 0x0, 0x3a1f10, 0x6c5250, 0x493227, 0x360e07, 0x865d46, 0xfff0db, 0xfff6f0, 0xffeae5, 0xffeade, 0xffede1, 0xffeadf, 0xfdebde, 0x3e3d47, 0x262b35, 0x222a2c, 0x1c281d, 0x141614, 0x1f1a25, 0x1a1b24, 0x80d12, 0x11111e, 0x1114, 0x87665b, 0xffc1bd, 0xd79b94, 0x7d5c55, 0x6e5b54, 0xaa847d, 0xe1b1a7, 0xe0b1af, 0xe7acaa, 0xb97373, 0xb46a69, 0xda9e94, 0xf3a2a4, 0xe56d83, 0xb15e64, 0x5f3b36, 0x321305, 0x461e16, 0xbc7970, 0xffb2a9, 0xffbdc3, 0xf5978f, 0xea908b, 0xf39c96, 0xf39e96, 0xed9690, 0xf9a49d, 0xffc5b9, 0xffc7c2, 0xffb4ae, 0xffa9a3, 0xffb1a4, 0xffb5a8, 0xfdb1a8, 0xffb1a7, 0xffaea4, 0xf9ac9c, 0xf7ac9a, 0xeaa29b, 0x8d615d, 0x60402, 0x0, 0x130400, 0x5e4037, 0x564d43, 0x4c3027, 0x875d46, 0xf1dcc9, 0xfffaf2, 0xfde9de, 0xffeae2, 0xffeae2, 0xffe9dd, 0xfbe9dc, 0x3b3b41, 0x33333c, 0x2d2a31, 0x232025, 0x151313, 0x181b22, 0x101a24, 0x6110e, 0x6060e, 0xd19, 0x634442, 0xffb3aa, 0xd39892, 0x735752, 0x6e5952, 0xaa8977, 0xd6b0a4, 0xe0b1af, 0xe0aca4, 0xc6847e, 0xbf7c74, 0xd29992, 0xef979e, 0xdc6278, 0xca7378, 0x724e40, 0x482620, 0x4b1915, 0x8d4548, 0xf9a8a4, 0xffbeb9, 0xeb9a90, 0xe48c89, 0xe9958f, 0xe6978b, 0xde8982, 0xe6978c, 0xfbb1a6, 0xffc0bb, 0xffb7b2, 0xffaea5, 0xffb0a1, 0xffb3a6, 0xfdb1a8, 0xfcaca5, 0xfcaa9e, 0xfaa899, 0xf4a395, 0xf3aa9b, 0xa76c69, 0x1a0403, 0x0, 0x10000, 0x3f2717, 0x634f43, 0x634640, 0x775b41, 0xecd6c1, 0xfff7ef, 0xffebdc, 0xffece2, 0xffe9e1, 0xffecde, 0xfeede1, 0x3e4041, 0x3f363e, 0x3a333f, 0x22232a, 0x111119, 0x1f2128, 0x12191f, 0x1c1a24, 0x5090a, 0x4141d, 0x2a1e2f, 0xd28d8a, 0xe2ac9a, 0x6d5b57, 0x584e46, 0xa28477, 0xcda9a3, 0xd6b1a7, 0xdea99d, 0xdf9f94, 0xcf9180, 0xd49790, 0xeb9192, 0xc55665, 0x924d4d, 0x5e3327, 0x38170a, 0x290d02, 0x44070c, 0xc7716e, 0xdb917f, 0xc17a74, 0xc47376, 0xc57870, 0xc17669, 0xc27064, 0xd88376, 0xf59d99, 0xffbfb3, 0xffc7be, 0xffb1a8, 0xffa59c, 0xfeaea2, 0xfcb1a1, 0xf8a799, 0xf6a394, 0xf4a596, 0xf2a294, 0xeea295, 0xa46464, 0x290206, 0x0, 0x0, 0x3c2717, 0x725e4d, 0x543d34, 0x8b6d54, 0xfee8d2, 0xffefe5, 0xffecdf, 0xffebdd, 0xffeadc, 0xffeadf, 0xffece3, 0x393b41, 0x353139, 0x3b3b44, 0x1e272c, 0x30404, 0x171922, 0x1f1e28, 0x201c28, 0x51112, 0xc171e, 0x61120, 0x734c4e, 0xefb19d, 0x856c61, 0x443d37, 0x94766b, 0xcca79f, 0xcda39b, 0xd79f95, 0xddaa92, 0xce907c, 0xce8b81, 0xd88c82, 0xa64d55, 0x793e39, 0x61332c, 0x2a0901, 0x160200, 0x631b25, 0xc46a69, 0xca8272, 0xc97675, 0xce7279, 0xcc7574, 0xc7776c, 0xd48274, 0xdb877c, 0xdb8a83, 0xe7a599, 0xfdbab4, 0xffb6af, 0xfca498, 0xfca59a, 0xffa79e, 0xfca797, 0xf6a797, 0xef9f91, 0xf19f92, 0xeca197, 0x8a5952, 0x1a0302, 0x0, 0x10000, 0x472d13, 0x705546, 0x593c34, 0xa48469, 0xfce3cc, 0xffede3, 0xfff0e4, 0xffefe2, 0xffefe1, 0xffede1, 0xffeae2, 0x30353f, 0x2c3139, 0x2b3237, 0x202730, 0x5050a, 0x90c0d, 0x1f2627, 0x161821, 0xf111c, 0xd0e16, 0x16282e, 0x1a1f22, 0xb18170, 0xbe9180, 0x4c393d, 0x785b53, 0xc19283, 0xc9988d, 0xd09a8d, 0xcf9c88, 0xc68a78, 0xcf8a79, 0xe2958b, 0xa3575f, 0x774143, 0x663326, 0x370504, 0x52130c, 0xb25b53, 0xf8a39e, 0xffaea4, 0xfc9689, 0xf3938b, 0xe8968a, 0xe58f8b, 0xea9297, 0xdf9289, 0xcf8c75, 0xb77b6b, 0xd99b94, 0xffbcb4, 0xfaaa9d, 0xf5a294, 0xfea79d, 0xfda79b, 0xf2a590, 0xef9b87, 0xee9c8e, 0xde998f, 0x7f5a51, 0x1c0a02, 0x0, 0x100100, 0x3b1905, 0x573b2f, 0x69493d, 0xad8873, 0xf9e4d2, 0xfff0e5, 0xffeee1, 0xffeee1, 0xffeddf, 0xffeee4, 0xffe9e4, 0x333748, 0x262c35, 0x283236, 0x20232e, 0xf0d16, 0x90906, 0x162021, 0x161f1c, 0x1f1920, 0x9060a, 0x1e2421, 0xa2924, 0x37332c, 0xb77f79, 0x896160, 0x553e3e, 0x9c7769, 0xc1937c, 0xcd9a87, 0xc9978a, 0xc1907c, 0xc89174, 0xe1998b, 0xae746e, 0x765349, 0x593027, 0x410609, 0xb0594b, 0xffbba9, 0xffc6c3, 0xffa9ab, 0xff9a96, 0xfa988e, 0xf29f95, 0xea9a97, 0xa98485, 0xc78a82, 0xefa593, 0x9b675c, 0xbd8478, 0xffb7b1, 0xf8a9a2, 0xf3a194, 0xf4a394, 0xf3a193, 0xee9e88, 0xe89882, 0xe59987, 0xcf8d7f, 0x784d47, 0x3a211a, 0x29211a, 0x190401, 0x3d190f, 0x5a3f35, 0x674b33, 0xc2a28d, 0xffefe3, 0xffefe1, 0xffe9de, 0xffe9dc, 0xfee8d8, 0xffebe1, 0xffeee5, 0x313640, 0x25272f, 0x292c33, 0x202830, 0x111412, 0x80609, 0xf1424, 0x1d2320, 0x1e2026, 0x6070b, 0x90b06, 0x1f2c29, 0x12161c, 0x473430, 0x9e7c6b, 0x745654, 0x765654, 0xb68979, 0xc59681, 0xcb998a, 0xd6a090, 0xb48670, 0x825c49, 0x795548, 0x674533, 0x4f2a24, 0x7d3d32, 0xffa99e, 0xffcdd0, 0xffb6b8, 0xffaba4, 0xffa29f, 0xf08e85, 0xf49c8e, 0xda9490, 0x473c3e, 0xad746e, 0xffbcba, 0xa1726d, 0xc58775, 0xf3a99f, 0xf0a59e, 0xefa092, 0xed9d89, 0xeb9988, 0xe59384, 0xdb8e7e, 0xe09583, 0xb87c6d, 0x472821, 0x321c20, 0x44302b, 0x311f0b, 0x553829, 0x5f3b2a, 0x7f5f49, 0xf2dfcd, 0xfff6ea, 0xfeeee1, 0xffece0, 0xffece0, 0xfeecdc, 0xfcecdd, 0xfceee2, 0x31343a, 0x2f2934, 0x2b282f, 0x222624, 0x171714, 0x30505, 0x30910, 0x2c2933, 0x24262c, 0x71018, 0x10706, 0x17211c, 0x382b31, 0x1e201d, 0x42392c, 0x7d5c52, 0x6d4e43, 0x8e6e61, 0xc49584, 0xcb9784, 0xd6a293, 0xba9584, 0x635244, 0x533935, 0x613e32, 0x8e5749, 0xf09f95, 0xffbeba, 0xffb1b3, 0xffb0ac, 0xffaea4, 0xfba7a0, 0xe99485, 0xeb947b, 0xd68b89, 0x764c56, 0xcc8a7d, 0xf9b6b3, 0xb07d72, 0xc88f76, 0xe7a294, 0xf0aaa1, 0xeaa295, 0xe59988, 0xe19281, 0xd28b7b, 0xcd8678, 0xd18a82, 0x6a3e3a, 0x20000, 0x332119, 0x472e25, 0x2e1905, 0x442d1b, 0x694636, 0xd0b6a1, 0xfffff3, 0xfff1e4, 0xfdebdc, 0xfcebdc, 0xffede1, 0xffede1, 0xfeeedf, 0xfcefe1, 0x383e42, 0x252b31, 0x2c312e, 0x2b282a, 0x19161a, 0x70a06, 0x50705, 0x1c191f, 0x32313a, 0x1e2430, 0x3130f, 0x60b05, 0x211419, 0x2b2827, 0x1e2421, 0x242421, 0x38352d, 0x3d2e25, 0x7c564b, 0xb48470, 0xc49583, 0xc7a292, 0xad8878, 0x9b6762, 0xb08078, 0xdea38f, 0xf6afa9, 0xf3a9a6, 0xf9afa5, 0xfcafa7, 0xf8a59d, 0xf2a099, 0xe99a8b, 0xe3997d, 0xb06865, 0x9d5458, 0xf2a99b, 0xdb9b9a, 0xbe7c71, 0xc6887a, 0xe1a397, 0xe4a39a, 0xdf9a8d, 0xda9286, 0xd48a79, 0xcc8974, 0xc98e81, 0x895d5c, 0x110203, 0x20000, 0x260e04, 0x3c2315, 0x37190d, 0x614134, 0xc7b69c, 0xfffcec, 0xf6e5d6, 0xe5d2c0, 0xf2ddce, 0xffeadd, 0xfcebde, 0xffeae0, 0xffebde, 0xffebdf, 0x2f3231, 0x1c282b, 0x292d2a, 0x363339, 0x27262a, 0x1c1a19, 0x90706, 0x40404, 0x282830, 0x30363e, 0x27292e, 0x161116, 0x90409, 0x191618, 0x29292a, 0x192022, 0x9120e, 0x20604, 0x1e120b, 0x6b4c45, 0x845f55, 0xa5776d, 0xc8948b, 0xd4978d, 0xd49991, 0xcc9890, 0xcd948a, 0xdd9895, 0xe7a09e, 0xea9f99, 0xe59a8f, 0xe69e94, 0xe9a79c, 0xd39882, 0x9c6258, 0xae6e6b, 0xca938b, 0xb17972, 0xc37d72, 0xd1938c, 0xcc958b, 0xcb8e7c, 0xce8973, 0xcd8677, 0xca897c, 0xb48072, 0x7a594f, 0x230e09, 0x30000, 0x70000, 0x170601, 0x251403, 0x3c2110, 0x8f6d5b, 0xd6c7b2, 0xddc2b6, 0xd8b7ab, 0xe0ceb9, 0xfeeadd, 0xffecdf, 0xffecdf, 0xfeece0, 0xffeadd, 0xffede1, 0x323136, 0x2e2b32, 0x29282a, 0x2b322e, 0x36373c, 0x2e2e30, 0xc0c0c, 0x60606, 0xf1112, 0x292930, 0x3b3840, 0x333236, 0x25282a, 0x1a2420, 0xf181d, 0x322a2b, 0x877664, 0x8f816e, 0xb3a28d, 0xc5c2bb, 0xa99f84, 0x947656, 0x986d5a, 0xa47864, 0xa87d6e, 0xa57a70, 0xaa8073, 0xb38178, 0xc18585, 0xcd8d88, 0xd6968e, 0xdba098, 0xd5a396, 0xc9937f, 0xc58a75, 0xbb8984, 0x946c61, 0xa2715f, 0xbc847a, 0xbd857d, 0xac756b, 0xa87667, 0xab7765, 0xab786c, 0x7c5552, 0x401c24, 0x37180f, 0x291004, 0x110100, 0x351309, 0x34170e, 0x5d4939, 0xc0a998, 0xd9c7ba, 0xe3d0c3, 0xecd2c7, 0xfde2d9, 0xfff2e3, 0xffeddf, 0xffebdd, 0xffeadf, 0xfdebde, 0xfaeede, 0xfdefe3, 0x414047, 0x36333a, 0x33363a, 0x283232, 0x35343e, 0x36363d, 0x191a15, 0x141011, 0x180d12, 0x141112, 0x272929, 0x31312f, 0x2b3831, 0x253d36, 0x2e3032, 0xb49882, 0xfff0cb, 0xffdbb3, 0xf3e0c9, 0xfcf8f3, 0xffedca, 0xecd6ae, 0xd1b595, 0xa3856d, 0xae7971, 0xbe867e, 0x9e8170, 0x998074, 0x9b7e72, 0x997a6d, 0xb5887f, 0xc1988e, 0xbe9388, 0xc28e7c, 0xbc8e7c, 0xa28072, 0x8c695f, 0x815a52, 0x7c564c, 0x795048, 0x845b51, 0x8f7165, 0x8a6b63, 0x715b4e, 0x56493d, 0x5e423c, 0x4c3124, 0x271002, 0x573e2f, 0x855b52, 0x6e3f31, 0xc1ad94, 0xffffff, 0xfff8f3, 0xfff1e5, 0xfff1e7, 0xffefe5, 0xfeecde, 0xfcebdb, 0xffeadf, 0xffe9e0, 0xffebde, 0xfbede0, 0xffede3, 0x3f4248, 0x34353e, 0x353947, 0x313741, 0x2e2e33, 0x2a272c, 0x1e1b1b, 0x100e06, 0x171111, 0x1d1a1d, 0x221e1f, 0x272422, 0x272925, 0x152122, 0x816d64, 0xfff0c8, 0xf0cea2, 0xe2c3a5, 0xf3f1f1, 0xfbefe5, 0xecc8a4, 0xebc8a9, 0xf6dabb, 0xb29684, 0xbd7870, 0xe3a099, 0xce9c92, 0xcb9792, 0xcf9995, 0xd59c99, 0x9e6a68, 0x75534e, 0x765351, 0x73534f, 0x513d30, 0x30200b, 0x1f0a02, 0x1b0500, 0x371108, 0x351c11, 0x2f2318, 0x3c291f, 0x563d36, 0x776155, 0x826d5d, 0x614431, 0x5a4021, 0x9a8c6f, 0xcbbca8, 0xb39784, 0xcebba0, 0xfffcea, 0xfff7ee, 0xfeeddc, 0xffeae0, 0xffebe2, 0xffeee0, 0xffede0, 0xfee9e0, 0xffeae2, 0xffede1, 0xffecdf, 0xffebe1, 0xffebe1, 0x32343b, 0x37373f, 0x373844, 0x33333f, 0x272628, 0x181815, 0x121316, 0xd0d0b, 0x141611, 0x1e201f, 0x292426, 0x32262e, 0x303234, 0x272621, 0xc09f87, 0xffe4b6, 0xe2bd9c, 0xf2e6e3, 0xffffff, 0xf1ddc2, 0xe9c39c, 0xe4c0a0, 0xf0d2ba, 0xe0dfdc, 0xbeada5, 0xb58278, 0xba7971, 0xca7f7a, 0xe68e8f, 0xf99fa4, 0xaf6b64, 0x663e2e, 0x502e26, 0x311712, 0xa0000, 0xc0300, 0x271903, 0x301c04, 0x270a02, 0x1c0600, 0x2a1b06, 0x523827, 0x755642, 0x8a6b59, 0x8d7359, 0xa2866e, 0xe2cfb7, 0xfffde8, 0xedd7c9, 0xe6ccbe, 0xfff8ea, 0xfff1ea, 0xffe9e2, 0xffefe3, 0xffebdc, 0xfeece0, 0xffeee2, 0xffece1, 0xfdebe0, 0xfeecdd, 0xfcefdd, 0xfeeddf, 0xffeadf, 0xfeeadf, 0x303333, 0x242d2b, 0x39363e, 0x2b2b33, 0x121218, 0x120e0d, 0x141113, 0xf1118, 0x1a1721, 0x26262a, 0x2c2d2d, 0x312733, 0x252730, 0x615549, 0xfcd5af, 0xeac39d, 0xe7d7ca, 0xffffff, 0xfbeff0, 0xeacbad, 0xe6c39f, 0xe4c09d, 0xf0d6cb, 0xffffff, 0xfefefc, 0xddbba1, 0xcc9e76, 0xd0a78b, 0xe1c0b8, 0xe9cbbd, 0xe3c2b4, 0xc5ad96, 0xaf8a76, 0xb1917f, 0xbea495, 0xcdb8a3, 0xdcc9bb, 0xd7bab2, 0xb6a18e, 0xc3b398, 0xead0bd, 0xf9e1d2, 0xefdece, 0xe9d6c9, 0xfbe4d2, 0xfffef0, 0xfffff4, 0xfeeadc, 0xfae2d4, 0xfff3e4, 0xfff1e4, 0xffe8dc, 0xfde9dd, 0xffebe5, 0xffeade, 0xfeecdd, 0xffede0, 0xffefe0, 0xfdefe0, 0xfeeddd, 0xffedde, 0xfceadf, 0xfee9de, 0xfdece0, 0x313436, 0x172426, 0x35353f, 0x201b27, 0x60611, 0xa0a0b, 0x141112, 0x161618, 0x19181e, 0x2e3334, 0x343737, 0x302b2c, 0x16111e, 0x947670, 0xffdfb3, 0xdfc0a3, 0xf8f2f0, 0xfcfbfe, 0xecdccf, 0xe4c0a0, 0xe5c6a5, 0xe2c39c, 0xeee1d9, 0xfaf8fb, 0xf3e6db, 0xf0cca5, 0xefc59a, 0xdec7ad, 0xf0e7de, 0xfffbe9, 0xfffced, 0xfff4e3, 0xffeada, 0xfffef0, 0xfffff9, 0xfffff6, 0xfffff4, 0xfffff7, 0xfffff7, 0xfffffb, 0xfffdf1, 0xfff7eb, 0xfff6ec, 0xfff7ec, 0xfff2e9, 0xffebe1, 0xfee9dd, 0xffeadd, 0xffefe5, 0xffecdf, 0xffeade, 0xffefe4, 0xffede1, 0xffe9e0, 0xffeadf, 0xffe8dc, 0xffecdf, 0xffede2, 0xffeadf, 0xfdebdd, 0xffeadc, 0xffe8e0, 0xffece4, 0xffeee3 }; 
	
static  void hubTestBmp(void) {
	memcpy(screen, testBmp, SCREEN_BYTES);
}	

static  void hubTestBmp2(void) {
	for (int y = 0; y < SCREEN_H; y++) {
		for (int x = 0; x < SCREEN_W; x++) {
			SCREEN[y][x] = COLOR(testBmp[SCREEN_W * y + x]);
		}
	}
}	

static  void hubTestBmp2H(void) {
	for (int y = 0; y < SCREEN_H; y++) {
		for (int x = 0; x < SCREEN_H; x++) {
			SCR.H[y][x] = COLOR(testBmp[SCREEN_W * y + x]);
		}
	}
}	

static int row, colorBit, colorI;

static enum hub_state {
	ST_PERIOD0 = 0,
	ST_PERIOD1,
	ST_PERIOD_DARK
} state;

static void initRows() {
	row = 0;
	colorBit = HUB_COLOR_BIT0;
	colorI = HUB_COLOR_I0;
	state = ST_PERIOD0;
}

int screenW, screenH;

static int screen_dx, screen_dy;
static int screen0;

static orient_t currentOrient = HUB_ROTATE_NOTSET;

void hubSetOrient(orient_t orient) {
	hubOrientation = orient;
	switch (hubOrientation) {
		case HUB_ROTATE_90:
			screenW = SCREEN_H;
			screenH = SCREEN_W;
			//screen_dx = -screenW;
			//screen_dy = 1;
			//screen0 = screenW * (screenH - 1);
			break;

		case HUB_ROTATE_180:
			screenW = SCREEN_W;
			screenH = SCREEN_H;
			//screen_dx = -1;
			//screen_dy = -screenW;
			//screen0 = SCREEN_LEN - 1;
			break;

		case HUB_ROTATE_270:
			screenW = SCREEN_H;
			screenH = SCREEN_W;
			//screen_dx = screenW;
			//screen_dy = -1;
			//screen0 = screenW - 1;
			break;

		case HUB_ROTATE_0:
		default:
			screenW = SCREEN_W;
			screenH = SCREEN_H;
			//screen_dx = 1;
			//screen_dy = screenW;
			//screen0 = 0;
			break;
	}
}

static inline void setOrientSwap() {
	if (hubOrientation != currentOrient) {
		currentOrient = hubOrientation;
		switch (currentOrient) {
			case HUB_ROTATE_90:
				//screenW = SCREEN_H;
				//screenH = SCREEN_W;
				screen_dx = -screenW;
				screen_dy = 1;
				screen0 = screenW * (screenH - 1);
				break;

			case HUB_ROTATE_180:
				//screenW = SCREEN_W;
				//screenH = SCREEN_H;
				screen_dx = -1;
				screen_dy = -screenW;
				screen0 = SCREEN_LEN - 1;
				break;

			case HUB_ROTATE_270:
				//screenW = SCREEN_H;
				//screenH = SCREEN_W;
				screen_dx = screenW;
				screen_dy = -1;
				screen0 = screenW - 1;
				break;

			case HUB_ROTATE_0:
			default:
				//screenW = SCREEN_W;
				//screenH = SCREEN_H;
				screen_dx = 1;
				screen_dy = screenW;
				screen0 = 0;
				break;
		}
	}
}

void hubInit(void) {
	initIO();
	calculatePeriod();
	hubLUTInit();
	initRows();
	hubOrientation = HUB_ROTATE_0;
	hubSetOrient(hubOrientation);
	clearScreen();
	//fillScreen(COLOR_RGB(255, 255, 255));
	//hubScreenTest();
	//hubGammaTest();
	hubTestBmp();
	//SCREEN[0][0] = COLOR_RGB( 255, 255, 255);
	/*for (int y = 0; y < 32; y++) {
		drawPixel(16, y, COLOR_RGB(0xff, 0, 0));
	}*/
	screenRedraw();
	
	// 100 Hz * 16 Phases
  HUB_TIMER_CLK_ENABLE();
  TIM_ClockConfigTypeDef sClockSourceConfig;

  hubtim.Instance = HUB_TIMER;
  hubtim.Init.Prescaler = HUB_PRESCALER - 1;
  hubtim.Init.CounterMode = TIM_COUNTERMODE_UP;
  hubtim.Init.Period = period0 - 1;
  hubtim.Init.ClockDivision = TIM_CLOCKDIVISION_DIV1;
  HAL_TIM_Base_Init(&hubtim);

  sClockSourceConfig.ClockSource = TIM_CLOCKSOURCE_INTERNAL;
  HAL_TIM_ConfigClockSource(&hubtim, &sClockSourceConfig);

	HAL_NVIC_SetPriorityGrouping(NVIC_PRIORITYGROUP_4);
  HAL_NVIC_SetPriority(TIM1_BRK_TIM9_IRQn, 0, 0);
  HAL_NVIC_EnableIRQ(TIM1_BRK_TIM9_IRQn);
	
	HAL_TIM_Base_Start_IT(&hubtim);
	
	// Timer 2 for NOE
	HUB_TIMER2_CLK_ENABLE();

  TIM_OC_InitTypeDef sConfigOC;

  hubtim2.Instance = HUB_TIMER2;
  hubtim2.Init.Prescaler = 0;
  hubtim2.Init.CounterMode = TIM_COUNTERMODE_UP;
  hubtim2.Init.Period = 0xffff;
  hubtim2.Init.ClockDivision = TIM_CLOCKDIVISION_DIV1;
  HAL_TIM_Base_Init(&hubtim2);

  HAL_TIM_OC_Init(&hubtim2);

  sConfigOC.OCMode = TIM_OCMODE_PWM1;
  sConfigOC.Pulse = period2Min;
  sConfigOC.OCPolarity = TIM_OCPOLARITY_LOW;
  sConfigOC.OCFastMode = TIM_OCFAST_DISABLE;
  HAL_TIM_OC_ConfigChannel(&hubtim2, &sConfigOC, NOE_TIM_CHANNEL);

	HAL_TIM_PWM_Start(&hubtim2, NOE_TIM_CHANNEL); 

}

static inline void outputRow(void) {
	pinWrite(hub_a, row & 1);
	pinWrite(hub_b, row & 2);
	pinWrite(hub_c, row & 4);
	pinWrite(hub_d, row & 8);
}

static 	color_t * buf, * buf2, * bufLine, * bufLine2;
static int period1n;

static inline void hubTask() {
	color_t pixel, pixel2;

	if (state == ST_PERIOD0) {
		//Period 0 - Shift row data
		pinReset(hub_time);
		//__HAL_TIM_SetAutoreload(&hubtim, period0 - 1);
		HUB_TIMER->ARR = period0 - 1;
		
		if (colorBit == HUB_COLOR_BIT0) {
			if (row == 0) {
				//Begin of Frame
				setBrightness();
				if (hubBrightness == 0) {
					//Display turn off
					//__HAL_TIM_SetAutoreload(&hubtim, FRAME_PERIOD - 1);
					HUB_TIMER->ARR = FRAME_PERIOD - 1;

					pinSet(hub_time);
					return;
				}
				if (hubNeedRedraw) {
					swapPages();
					setOrientSwap();
				}	
				bufLine = &DISPLAY[screen0];
				bufLine2 = &bufLine[HUB_ROWS * screen_dy];
			}
			outputRow();
		}
	
		int red, green, blue;
		int red2, green2, blue2;
		buf = bufLine;
		buf2 = bufLine2;
		for (int x = 0; x < SCREEN_W; x++) {
			pixel = *buf;
			red = hubLUT[pixel.R];
			green = hubLUT[pixel.G];
			blue = hubLUT[pixel.B];

			pixel2 = *buf2;
			red2 = hubLUT[pixel2.R];
			green2 = hubLUT[pixel2.G];
			blue2 = hubLUT[pixel2.B];

			pinReset(hub_clk);
			
			pinWrite(hub_r1, red & colorBit);
			pinWrite(hub_g1, green & colorBit);
			pinWrite(hub_b1, blue & colorBit);
			
			pinWrite(hub_r2, red2 & colorBit);
			pinWrite(hub_g2, green2 & colorBit);
			pinWrite(hub_b2, blue2 & colorBit);
			
			buf += screen_dx;
			buf2 += screen_dx;
		
			pinSet(hub_clk);
		}
		period1n = period1N[colorI];
		prescaler2n = prescaler2N[colorI];
		colorBit >>= 1;
		colorI--;
		if (colorBit == 0) {
			colorBit = HUB_COLOR_BIT0;
			colorI = HUB_COLOR_I0;
			//Change Row
			row = (row + 1) & HUB_ROWS_MASK;
			bufLine += screen_dy;
			bufLine2 += screen_dy;
		}	
		pinSet(hub_lat);
		state = ST_PERIOD1;
		pinReset(hub_lat);
	} else if (state == ST_PERIOD1) {
		//Period 1 - Show current row
		
		//Set prescaler
		HUB_TIMER2->PSC = prescaler2n - 1;
		//Update Event Generation
		HUB_TIMER2->EGR = TIM_EGR_UG;
		//Enable Timer
		HUB_TIMER2->CR1 |= TIM_CR1_CEN;
		//Set Period
		//__HAL_TIM_SetAutoreload(&hubtim, period1n - 1);
		HUB_TIMER->ARR = period1n - 1;
		state = ST_PERIOD0;
		if ((colorBit == HUB_COLOR_BIT0) && (row == 0)) {
			//Last pixels
			state = ST_PERIOD_DARK;
		}
	}	else if (state == ST_PERIOD_DARK) {
		//Period Dark - End of frame
		//Set period
		HUB_TIMER->ARR = periodDark - 1;
		//Disable Timer
		HUB_TIMER2->CR1 &= ~TIM_CR1_CEN;

		state = ST_PERIOD0;
	}

	pinSet(hub_time);
}

void TIM1_BRK_TIM9_IRQHandler(void)
{
	//if (__HAL_TIM_GET_ITSTATUS(&hubtim, TIM_IT_UPDATE) != RESET) {
	if ((HUB_TIMER->DIER & TIM_IT_UPDATE) != 0) {	
		hubTask();
		//__HAL_TIM_CLEAR_IT(&hubtim, TIM_IT_UPDATE);
		(HUB_TIMER->SR = ~(TIM_IT_UPDATE));
	}
	//HAL_NVIC_ClearPendingIRQ(TIM1_BRK_TIM9_IRQn);
	NVIC->ICPR[TIM1_BRK_TIM9_IRQn >> 5] = 1 << TIM1_BRK_TIM9_IRQn;
}

void inline clearScreen(void) {
	memset(screen, 0, SCREEN_BYTES);
}

void inline copyScreen(screen_t * dst, screen_t * src) {
	memmove(dst, src, SCREEN_BYTES);
}

void fillScreen(color_t color) {
	color_t * sc = (color_t *)screen;
	for (int i = 0; i < SCREEN_LEN; i++) {
		*sc = color;
		sc++;
	}
}


