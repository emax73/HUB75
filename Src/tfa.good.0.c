...
static enum {
	...
	ST_HEADER,
	ST_DATA
} state = ST_NOSIGNAL, state0 = ST_NOSIGNAL;
...
static char data_c[PACKETS_NUM][PACKET_LEN] = { ' ' };
...
static char badBits[PACKETS_NUM][PACKET_LEN_B] = { 0 };
int tfaLevel = 0; // 0 - Lost, 1 - Weak, 2 - Middle, 3 - Good
...
static int duty_stat[101] = { 0 };
static short pulse_stat[1001] = { 0 };
static short period_stat[901] = { 0 };

#define PULSE_LEN 1000
static int pulse_pause[PULSE_LEN][4] = { 0 };
static int pulseI = 0;
static int pulseIp[PACKETS_NUM] = { 0 };
...
//#define PACKAGES_EQUAL(i, j) (data[(i)][0] == data[(j)][0] && data[(i)][1] == data[(j)][1] && data[(i)][2] == data[(j)][2] && data[(i)][3] == data[(j)][3] && data[(i)][4] == data[(j)][4])
...
#define PACKAGES_DIFF(i, j) badBits[(i)][0] = data[(i)][0] ^ data[(j)][0]; badBits[(i)][1] = data[(i)][1] ^ data[(j)][1]; badBits[(i)][2] = data[(i)][2] ^ data[(j)][2]; badBits[(i)][3] = data[(i)][3] ^ data[(j)][3]; badBits[(i)][4] = data[(i)][4] ^ data[(j)][4]

#define TFA_LEVEL_LOST_US (10 * 60 * 1000000) // 30 min for level 0
...
static char data0[12] = { ' ' };
static char data0I[12] = { ' ' };
static char data30[10] = { ' ' };
static char data30I[10] = { ' ' };
static char badData[12] = { 0 };

static unsigned char getCrc(unsigned char *pcBlock, int len, unsigned char poly)
{
...
static unsigned char getCrc2(unsigned char *pcBlock, int len)
{
    unsigned char crc = 0x00;
...
static int getCrc3(char *data, int len)
{
    int crc = 0;
...
static void comparePackets(void) {
	...
	if (max_v >= 3) {
		...
		//CRC
		/*char crc1 = getCrc3(dataOk, 5);
		if (!crc1) {
			test = 1;
		}
		char crc2 = dataOk[4];
		int poly = 0x7c;
		//for (int poly = 1; poly < 256; poly++) {
			crc1 = getCrc2((unsigned char *)dataOk, 4);
			if (crc1 == crc2) {
				test = 1;
			}
		//}*/
...
static int bitPeriod = 700;
static int packetLens[PACKETS_NUM];

static void firstPacket(void) {
			memset(pulse_pause, 0, sizeof(pulse_pause));
			memset(data, 0, sizeof(data));
			memset(data_c, ' ', sizeof(data_c));
			memset(data30, ' ', sizeof(data30));
			memset(data30I, ' ', sizeof(data30I));
			memset(pulseIp, 0, sizeof(pulseIp));
			memset(duty_stat, 0, sizeof(duty_stat));
			memset(pulse_stat, 0, sizeof(pulse_stat));
			memset(period_stat, 0, sizeof(period_stat));
...
/*void tfaTask(void) {
	...
	if ((level && !level0) || ((pause >= MAX_LEN_US) && (pause0 < MAX_LEN_US))) {	//Front
		...
		//Log
		if (pulseI < PULSE_LEN) {
			pulse_pause[pulseI][0] = pulse;
			pulse_pause[pulseI][1] = pause;
			pulse_pause[pulseI][2] = period;
			pulse_pause[pulseI][3] = duty;
			pulseI++;
		}

		//if ((period >= 1000) && (period <= 2000) && (pause >= 700) && (pause <= 1000)) {	//Sync
		if ((pulse >= 550) && (pulse <= 1100) && (((pause >= 550) && (pause <= 1100)) || (pause >= MAX_LEN_US))) {	//Sync
			if (sync_i < SYNC_NUM)	{
				sync_i++;
			}	
			if (sync_i == 1) {
			...
			if ((state != ST_SYNC) && ((sync_i >= SYNC_NUM) || ((sync_i >=2) && (pause >= MAX_LEN_US)))) {
				...
				//prevSync = 0;
		//} else if ((period >= 600) && (period <= 800)) {	//Data
		//} else if ((period >= 500) && (period <= 850)) {	//Data
		//} else if ((period >= 150) && (period <= 850)) {	//Data
			...
			if (state == ST_SYNC) {
				...
				state = ST_HEADER;
				data_i = 0;
				if (packets_i < PACKETS_NUM) pulseIp[packets_i] = pulseI - 1; //Time flag
			}
			if (state == ST_HEADER) {
	...
	if ((level && !level0) || ((pause >= MAX_LEN_US) && (pause0 < MAX_LEN_US))) {	//Front
		...
		//} else if ((period >= 150) && (period <= 850)) {	//Data
			...
			if (state == ST_DATA) {
					if (data_i < PACKET_LEN) {
						SET_DATA(packets_i, data_i, duty >= bitTreshold);
						//SET_DATA(packets_i, data_i, pulse >= bitTreshold);
						data_i++;
					}
 			}
			sync_i = 0;
